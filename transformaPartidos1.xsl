...
<body>
	...
	<tbody>
		<xsl:for-each select="partidos/partido">
			<xsl:if test="substring(./Resultado,1,1) &lt; substring(./Resultado,3)">
				<tr>
			...
			<xsl:if test="substring(./Resultado,1,1) &gt; substring(./Resultado,3)">
				<tr>
					<td><xsl:value-of select="Ronda"/></td>
					<td><xsl:value-of select="Fecha"/></td>	
					<td id="ganador"><xsl:value-of select="EquipoLocal"/></td>
					<td><xsl:value-of select="Resultado"/></td>	
					<td id="perdedor"><xsl:value-of select="EquipoVisitante"/></td>
				</tr>
			</xsl:if>
			<xsl:if test="substring(./Resultado,1,1)=substring(./Resultado,3)">
				<tr>
					<td><xsl:value-of select="Ronda"/></td>
					<td><xsl:value-of select="Fecha"/></td>	
					<td id="empate"><xsl:value-of select="EquipoLocal"/></td>
					<td><xsl:value-of select="Resultado"/></td>	
					<td id="empate"><xsl:value-of select="EquipoVisitante"/></td>
				</tr>
			</xsl:if>
		</xsl:for-each>	
	</tbody>
